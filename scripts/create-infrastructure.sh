...
{
  start_log=$(
    start_vm 10.4.1.91 associate-public-ip-address ${USER_NAME}-vm1 file://<( initial_command )
  )

  instance_id=$(echo "${start_log}" | jq -r .Instances[0].InstanceId)

  # for i in {2..3}; do
  #   start_vm 10.9.1.$((90+i)) no-associate-public-ip-address ${USER_NAME}-vm$i > /dev/null
  # done
